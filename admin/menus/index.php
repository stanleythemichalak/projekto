...
echo "
<form method='POST' action='index.php?module=menus&zatwierdz=".$_GET['edytuj']."'>
         <h3 class='tit'>Edytujesz Link: <u>$row->tytul_en</u></h3>               		   
  <fieldset>				     
    <legend>    
    </legend>	
    <table class='nostyle'>		
...
?>
     ...
     <form method='POST' action='index.php?module=menus&action=add' enctype="multipart/form-data"> 
        <h3 class="tit">Jesteś teraz w trybie zarządzania linkami</h3>               		   
  <fieldset>				     
    <legend>    
    </legend>	
    <table class="nostyle">		
...
$sql = "INSERT INTO linki(tytul_en,tytul_pl,podtytul_en,podtytul_pl,przypisanie,poziom,kolejnosc,link_en,link_pl) 
VALUES('".mysql_real_escape_string($_POST['tytul_en'])."','".mysql_real_escape_string($_POST['tytul_pl'])."',
'".mysql_real_escape_string($_POST['podtytul_en'])."','".mysql_real_escape_string($_POST['podtytul_pl'])."','".$_POST['przypisanie']."','".$poziom."','".$ilosc."','".$_POST['link_en']."','".$_POST['link_pl']."')";
$result = mysql_query($sql) or die(mysql_error());   
...
$str.=menu($row->id, $next_poziom);
 ...
 echo ' <fieldset>				     
    <legend>    
    </legend>	<table class="nostyle"><tr><td class="va-top">Dostępne Linki:</td>'; 
echo'<td><ul>';
 echo menu(0,0);
